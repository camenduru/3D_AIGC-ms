...
def model_preview_url(url, title, biz_usage):
    if not url:
        return ""
...
def single_model_viewer_iframe(url, glb_url, title, biz_usage, show_actions: bool):
    iframe = f"""<iframe class='model_container' src ='{model_preview_url(url, title, biz_usage)}'></iframe>"""
    ...
    if share_action:
        share_action["onclick"] = f"shareSDModelClick('{model_preview_url(url, title, biz_usage)}')"

    return str(soup)


# project list 格式：参看api_manager.parse_project_list返回值
def project_list_html(project_list, biz_usage):
    if not project_list:
