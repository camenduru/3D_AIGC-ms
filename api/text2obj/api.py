...
from alibabacloud_xrengine20230313.models import PopCreateObjectGenerationProjectRequest, PopBuildObjectGenerationProjectRequest, PopQueryObjectGenerationProjectDetailRequest, PopBatchQueryObjectGenerationProjectStatusRequest, PopListObjectGenerationProjectRequest
import datetime
import json
import time
...
async def text2Substance(jwt, prompt):
  try:
    ...
    if create_response.status_code == 200 and create_response.body.success is True:
      ...
      build_response = await xr_client.pop_build_object_generation_project_async(PopBuildObjectGenerationProjectRequest().from_map({"JwtToken": jwt, "ProjectId": project_id}))
...
async def queryText2ObjModelDetail(id):
  try:
    result = (await xr_client.pop_query_object_generation_project_detail_async(PopQueryObjectGenerationProjectDetailRequest().from_map({"ProjectId": id}))).to_map()
    ...
    if result['statusCode'] == 200 and result["body"]['success'] is True:
      ...
      if (status == 'VIEWABLE' or (dataset is not None and dataset['buildResultUrl']['whiteModel'] is not None)):
        return {"timestamp": str(datetime.datetime.now().time()), "success": True, "id": id, "status": result['body']['data']['status'], "ext": result["body"]['data']['ext'], "dataset": result['body']['data']['dataset']}
      else:
        return None  
      ...
      if is_json(err) is False:
        err = '未知错误'
      return {"timestamp": str(datetime.datetime.now().time()), "success": False, "id": id, "err": err, "dataset": "", "status": "MAKING_FAILED"}
    return None
  except Exception as e:
...
def queryText2ObjModelsDetail(ids):
  ids = ','.join(ids)
  try:
    result = (xr_client.pop_batch_query_object_generation_project_status(PopBatchQueryObjectGenerationProjectStatusRequest().from_map({"ProjectIds": ids}))).to_map()
    result = transformResponse(result)
    if (result["statusCode"] == 200 and result["body"]["success"] is True):
